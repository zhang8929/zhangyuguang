...
def prepare_jb(file1,file2):
    ...
    with open(file1,'r',encoding='utf-8') as file:
        ...
        for every_dict in json_data:
            ...
            alias_list = every_dict.get('别名：','暂无数据').strip().split(' ')
            tmp['别称']=[]
            for x in alias_list:
                x.replace('"',"'")
                tmp['别称'].append(x)
            ...
            tmp['治疗方法详述'] = every_dict.get('治疗方法详述','暂无数据')
            ...
            tmp['检查内容详述'] = every_dict.get('检查内容：','暂无数据')
            tmp['病症详述'] = every_dict.get('症状内容：','暂无数据')
...
if __name__ == '__main__':
    prepare_jb('/Users/zhangyuan/work/爬虫数据/9939/rest.json','/Users/zhangyuan/work/爬虫数据/9939/rest_ready.json')