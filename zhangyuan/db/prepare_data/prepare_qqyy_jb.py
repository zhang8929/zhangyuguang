...
def prepare_jb(file1,file2):
    ...
    with open(file1,'r',encoding='utf-8') as file:
        ...
        for every_dict in json_data:
            # print(every_dict['名称'])
            tmp={}
            tmp['名称'] = every_dict.get('title','')
            tmp['英文名'] = every_dict.get('英文名','暂无数据')
            tmp['定义'] = every_dict.get('疾病介绍','暂无数据')
            tmp['别称'] = every_dict.get('疾病别名','暂无数据').strip().split(' ')
            tmp['科室'] = every_dict.get('就诊科室','暂无数据').strip().split(' ')
            tmp['部位'] = every_dict.get('所属部位','暂无数据').strip().split(' ')
            tmp['症状'] = every_dict.get('症状体征','暂无数据').strip().split(' ')
            ...
            tmp['治疗方法详述'] = every_dict.get('治疗方案','暂无数据')
            ...
            tmp['鉴别详述'] = every_dict.get('鉴别诊断','暂无数据')
            tmp['预防详述'] = every_dict.get('预防详述','暂无数据')
            tmp['饮食保健'] = every_dict.get('饮食保健','暂无数据')
            tmp['检查内容详述'] = every_dict.get('检查化验','暂无数据')
...
if __name__ == '__main__':
    prepare_jb('/Users/zhangyuan/work/爬虫数据/全球医院/qqyy-jibing.json','/Users/zhangyuan/work/爬虫数据/全球医院/qqyy_jibing_ready.json')