...
class TripleExtractor:
    ...
    def ruler1(self, words, postags, roles_dict, role_index):#2
        v = words[role_index].decode('utf-8')
        role_info = roles_dict[role_index]#{'A1': ['A1', 3, 3], 'ADV': ['ADV', 1, 1], 'A0': ['A0', 0, 0]}
        if 'A0' in role_info.keys() and 'A1' in role_info.keys():
            s = ''.join([words[word_index].decode('utf-8') for word_index in range(role_info['A0'][1], role_info['A0'][2]+1) if
                         postags[word_index][0] not in ['w', 'u', 'x'] and words[word_index].decode('utf-8')])#我
            o = ''.join([words[word_index].decode('utf-8') for word_index in range(role_info['A1'][1], role_info['A1'][2]+1) if
                         postags[word_index][0] not in ['w', 'u', 'x'] and words[word_index].decode('utf-8')])#头晕
            if s  and o:
                return '1', [s, v, o]#主语谓语宾语
    ...
    def ruler2(self, words, postags, child_dict_list, arcs, roles_dict):
        svos = []
        for index in range(len(postags)): #['r', 'd', 'v', 'v'] 0,1,2,3
            tmp = 1
            # 先借助语义角色标注的结果，进行三元组抽取
            if index in roles_dict:#键是动词的index
                flag, triple = self.ruler1(words, postags, roles_dict, index)#1，【我感到头晕】
                if flag == '1':
                    svos.append(triple)
                    tmp = 0
            if tmp == 1:
                ...
                # 抽取以谓词为中心的事实三元组
                    ...
                    if 'SBV' in child_dict and 'VOB' in child_dict:
                        r = words[index].decode('utf-8')#感到
                        e1 = self.complete_e(words, postags, child_dict_list, child_dict['SBV'][0])#宾语
                        e2 = self.complete_e(words, postags, child_dict_list, child_dict['VOB'][0])#主语
                        svos.append([e1, r, e2])
                    ...
                    if 'SBV' in child_dict and 'CMP' in child_dict:
                        ...
                        r = words[index].decode('utf-8') + words[cmp_index].decode('utf-8')
    ...
    def complete_e(self, words, postags, child_dict_list, word_index):
        child_dict = child_dict_list[word_index]#{}，{'ADV': [1], 'SBV': [0], 'VOB': [3]}
        prefix = ''
        ...
        return prefix + words[word_index].decode('utf-8') + postfix
    ...
    def triples_main(self, content):
        ...
        for sentence in sentences:
            words, postags, child_dict_list, roles_dict, format_parse_list,vWords,arcs = self.parser.parser_main(sentence)
