...
with open('./jkyx_ks_href.txt', 'r', encoding='utf-8') as f:
    data = f.readlines()
    # print(data)

# 26857页/20条问答
for url in data[0:10000]:
    print(111, url.strip())
    try:
        ...
        li_url_list = html.xpath('//*[@class="ask-list mg-b28"]/li/a/@href')
        print(li_url_list)

        for li_url in li_url_list:
            url2 = "https://www.vodjk.com"+ li_url +""
            # print(url2)
            try:
                r2 = requests.get(url2, headers=headers)
                r2 = r2.text
                html2 = etree.HTML(r2)
                questions_list = html2.xpath('//*[@class="newaskbox"]/div[2]/div/text()')
                # print(questions_list)

                ask_dic = {}
                for question in questions_list:
                    ask_dic["question"] = question.strip()
                    # print("qqqqqq", question.strip())

                if len(questions_list) != 0:
                    try:
                        answer_list = html2.xpath('//*[@class="fl-left w710"]/div[2]/div[1]/div[1]/p[1]/text()')
                        # print(answer_list)
                        for answer in answer_list:
                            ask_dic["answer"] = answer.strip()
                            # print("aaaaaa", answer.strip())
                        if len(answer_list) != 0:
                            ask_dic_json = json.dumps(ask_dic, ensure_ascii=False)
                            print(ask_dic_json)
                            with open('./jkyx_ask.json', 'a+', encoding='utf-8') as f:
