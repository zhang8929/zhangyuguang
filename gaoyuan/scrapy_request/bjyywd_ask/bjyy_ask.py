...
with open('./bjyy_href.txt', 'r', encoding='utf-8') as f:
    data = f.readlines()
    # print(data)
n = 15980
for url in data[15980:]:
    print(n, url.strip())
    n += 1
    url = url.strip()
    ...
    try:
        ...
        ask_dic = {}
        try:
            questions_list = html.xpath('//*[@id="d_msCon"]/p/text()')
            ...
            if q != "":
                ask_dic["question"] = q
                try:
                    answer_list = html.xpath('//*[@class="q_a da q_a_cn"]/div[@class="qas"]/dl/dd/p/text()')
                    # answer_list = html.xpath('//*[@class="c_left"]/div[3]/div[@class="qas"]/dl/dd/p//text()')
                    # print(answer_list)
                    ...
                    for answer in answer_list:
                        # print(answer)
                        a += answer.strip().replace("病情分析：", "").replace("指导意见：", "")
                    # print(a)
                    if a != "":
                        ask_dic["answer"] = a
                        # print(ask_dic)
                        ...
                        print(ask_dic_json)
                        with open('./bjyy_ask.json', 'a+', encoding='utf-8') as f:
