...
source = open("/zhangyuguang/shuju/data/source.txt", 'w')
target = open("/zhangyuguang/shuju/data/target.txt", 'w')

with open("/zhangyuguang/shuju/data/对联.txt", 'r') as f:
    lines = f.readlines()
    for line in lines:
...
with open('/zhangyuguang/shuju/data/source.txt', 'r', encoding='utf-8') as f:
    source_data = f.read()

with open('/zhangyuguang/shuju/data/target.txt', 'r', encoding='utf-8') as f:
    target_data = f.read()
...
with tf.Session(graph=loaded_graph) as sess:
    ...
    print('USE_TIME : {}'.format(time.time()-t1))