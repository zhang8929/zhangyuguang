...
def news_list(category_id):
    ...
    #     list3.append({
    #         'id': news.id,
    #         'pic_url': news.pic_url[0],
    #         'title': news.title,
    #         'summary': news.summary[:50]+'【详情点击...】',
    #
    ...
    # 接收新闻分类的编号category_id
    # 查询指定分类的所有新闻数据
    list1 = NewsInfo.query.order_by(NewsInfo.update_time.desc())  # select * from newsinfo
    if category_id > 0:
        # 如果选择的是“最新”==0，表示所有分类的新闻
        list1 = list1.filter_by(category_id=category_id)  # where ....
    # 排序：最新的显示在最前面
    ...
    for news in list2:
        ...
        new = "./static/news/images/yl/" + news.pic.replace('[', '').replace(']', '').replace("'", '').split(',')[0]
        if "./static/news/images/yl/" == new:
            new = "./static/news/images/yl/乙肝疫苗该怎么打？1.jpeg"

        # path = "./static/news/images/yl/" + news.pic[0]
        ...
        list3.append({
            ...
            # 'pic': news.pic,
            'title': news.title,
            'summary': news.summary[:50]+u'【详情点击...】',
            # 'create_time': news.create_time.strftime('%Y-%m-%d'),  #将日期转成字符串，并指定格式
            # 'avatar_url': news.user.avatar_url,
            'nick_name': news.user.nick_name,
...
def ico():
    ...
    return news_blueprint.send_static_file('news/images/favicon.ico')
...
def detail(news_id):
    ...
    #     g.user = None
    # # 查询点击量排行
    # click_list = NewsInfo.query.order_by(NewsInfo.click_count.desc())[0:10]
    #
    # # 响应
    # return render_template(
    #     'news/detail.html',
    ...
    news_pic = news.pic.replace('[', '').replace(']', '').replace("'", '').replace("'", '')
    news.pic1 = news_pic.split(',')
    
    for i in news.pic1:
        if len(i) > 0:
            path = "./static/news/images/yl/" + i.strip()
...
@news_blueprint.route('/search', methods=['GET','POST'])
def search():
    keyword = request.args.get('keyword')
    print('======')
    result = NewsInfo.query \
        .filter(NewsInfo.title.contains(keyword)).order_by(NewsInfo.id.desc())
    print(result)
    if result:
        print('xxxxxxxxxxxxxxxx')
        return render_template('news/base.html', news=result)
    else:
        return 'NOT FOUND'